...
def filter_df (df : pd.DataFrame, high_p_ind:tuple) -> pd.DataFrame:
    ...
    return df_filtered.drop( columns = df_filtered.columns[1:10], axis = 1) # drop info on cage   ### try if can drop axis = 1
...
def calculate_and_add_mouse_CM(df_filtered : pd.DataFrame) -> pd.DataFrame:
    ...
    columns : list = [[[value,coord] for value in body_parts] for coord in coord_list] 
    
    for i,coord in enumerate(coord_list):
        df_filtered['CM',coord] = df_filtered[columns[i]].mean(axis = 1)

    # plot CM x,y points - note: floor  90 degrees rotation anticlockwise 
    return df_filtered


def get_cage_coord_full(df : pd.DataFrame) -> dict:

    side_length_cm : float = 33
    cage_coord_full : dict = {}
    ...
    scale_pix_to_cm : float = get_scale_pix_to_cm(cage_coord, side_length_cm)


    
    cage_coord_normalized : dict = {key : v * scale_pix_to_cm for key, v in cage_coord.items()} 
    zero: dict = get_zero_coord(cage_coord_normalized)
    cage_coord_normalized = {k : v - zero[k[1]] for k,v in cage_coord_normalized.items()}

    cage_coord_full['scale_pix_to_cm'] = scale_pix_to_cm
    cage_coord_full['zero'] = zero
    cage_coord_full['cage_coord_normalized'] = cage_coord_normalized

    return cage_coord_full
...
def cal_side_length_in_pix( cage_coord : dict ):
    ...
    def get_side_lengths_pix(corners):
        return (  ( cage_coord[ corners[0],'x'] - cage_coord[ corners[1], 'x'] ) ** 2 +                 
    ...
    side_lengths_pix_v2 = [get_side_lengths_pix(corner_pair) for corner_pair in corner_pairs if sum]
...
def scale_df_vals_to_cm(df : pd.DataFrame, scale_pix_to_cm : int, coord_list : list = ['x', 'y']) -> pd.DataFrame:
    
    ''' multiply each pixel value of each coordinate and label pair by the scale'''
    label_list : list = list(set([key[0] for key in df.columns if key[0] != 'bodyparts'])) 
    coord_label_pair : list = [(label,coord) for label in label_list for coord in coord_list]
    df[coord_label_pair] = df[coord_label_pair].apply(lambda x: x * scale_pix_to_cm)
    return df


def get_scale_pix_to_cm(cage_coord : dict, side_length_cm : float) -> float:
    side_length_pix : float = cal_side_length_in_pix( cage_coord )
    return cal_scale_pix_to_cm(side_length_cm, side_length_pix)
