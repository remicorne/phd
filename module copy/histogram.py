...
def histogram(
    ...
    data, order, palette = buildHistogramData(filename, experiment, compound, region)
    if not data.eliminated_outlier.notna().all():
        processOutliers(
            filename,
            experiment,
            compound,
...
def buildHistogram(
    title, ylabel, data, order, palette, hue=None, significance_info=None
):
    ...
    if significance_info:
        ax = labelStats(ax, data, x, y, order, significance_info)
...
def labelStats(ax, data, x, y, order, significance_info):
    pairs, p_values = significance_info
