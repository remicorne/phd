...
def buildCompoundDf(filename):
    ...
    return new_format_df.rename(columns={'value': 'ng_mg'}).drop(columns=['variable'])
...
def buildRatiosDf(filename):
    ...
    merged[['compounds', 'ratio']] = merged.apply(lambda x: [f'{x.compound_1}/{x.compound_2}', x.ng_mg_1 / x.ng_mg_2 if x.ng_mg_2 else np.NaN], axis=1, result_type='expand') #calculate the ratio
...
def isSignificant(stat_method_cb, pval_threshold): #This is a classic design pattern of which i forget the name again. 
...
def getPeasonCorrStats(df, pivot_column, p_value_threshold, n_minimum):
    ...
    pivot_df = df.pivot_table(values='ng_mg', index=df['mouse_id'], columns=pivot_column).filter(COLUMN_ORDER[pivot_column]) #Here we just picot our structure to one where each column is a region or compound and each line a mouse
...
def getSubselection(df, selector):
    query = []
    for key, values in selector.items():
        query += [f"{key}=='{value}'" for value in values]
...
def getAndPlotCorrelograms(filename, selector, p_value_threshold=0.05, n_minimum=5): #TODO: Improve performance cuz this is slow AF
    ...
    for column, values in selector.items(): #Iterate through the selector dict
        ...
        compound_df = getSubselection(compound_df, selector)
        # subselection_df = pd.concat([compound_df[compound_df[column] == value] for value in values])
        for grouping_name, column_groupby_df in compound_df.groupby(by=[column]): #TODO group all graph of on experiment
            for experiment, experiment_groupby_df in column_groupby_df.groupby(by=['experiment']): 
                to_plot = []
                for treatment, treatment_groupby_df in experiment_groupby_df.groupby(by=['treatment']): 
                    pivot_column = 'region' if column == 'compound' else 'compound'
                    # correlogram_df, p_value_mask = getPeasonCorrStats(treatment_groupby_df, pivot_column, p_value_threshold, n_minimum)
                    # plotCorrelogram(correlogram_df, p_value_mask, f"{grouping_name[0]} in {treatment[0]}")
                    to_plot.append([*getPeasonCorrStats(treatment_groupby_df, pivot_column, p_value_threshold, n_minimum), f"{grouping_name[0]} in {treatment[0]}"])
...
def plotCorrelograms(correlograms):
    ...
    for (correlogram_df, p_value_mask, title), ax in zip(correlograms, axs):
        plotCorrelogram(correlogram_df, p_value_mask, title, ax)
    fig #plot

def plotCorrelogram(correlogram_df, p_value_mask, title, ax):
    heatmap = sns.heatmap(correlogram_df, vmin=-1, vmax=1, annot=True, cmap='BrBG', mask=p_value_mask, annot_kws={"size": 8}, ax=ax)
    heatmap.set_title(title, fontdict={'fontsize': 10}, pad=20)
