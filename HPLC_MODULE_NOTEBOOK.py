...
def getHTHistogram(filename, HT_filename, experiment='dose_responce', p_value_threshold=0.05, to_plot=[], from_scratch=None):
    identifier = f"HT_Histogram_{experiment}_for_{to_plot}"
    from_scratch = from_scratch if from_scratch is not None else input("Recalculate figure even if previous version exists? (y/n)") == 'y'
    if from_scratch or not isCached(filename, identifier):
        fig = buildHTHistogram(filename, HT_filename, experiment=experiment, p_value_threshold=p_value_threshold, to_plot=to_plot)
