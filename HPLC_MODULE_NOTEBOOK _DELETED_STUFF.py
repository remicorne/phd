...
def saveFigure(fig, identifier, fig_type):
    ...
    fig.savefig(
        f"{output_subdir}/{identifier}.png", dpi=fig.dpi
...
def applyTreatmentMapping(df, filename):
    ...
    new_columns = list(
        list(treatment_mapping.values())[0].keys()
    ...
    return df.explode("experiments").rename(
        columns={"experiments": "experiment"}
...
def buildAggregateStatsDf(filename, df_type):
    # CHANGED
    working_df = getCompoundAndRatiosDf(
        filename
    ...
    for treat_region_comp, groupby_df in working_df.groupby(
        ...
        result_ls.append(
            [*treat_region_comp, F, p, is_valid, mean, std, sem, values]
...
def isSignificant(
    stat_method_cb, pval_threshold=0.05
...
def getTwoWayAnova(data, experimental_info):
    data[experimental_info["independant_vars"]] = data.apply(
        lambda x: [
            var in x["treatment"] for var in experimental_info["independant_vars"]
        ],
        axis=1,
        result_type="expand",
    )
    ...
    return pg.anova(
        data=data,
        dv="value",
        between=experimental_info["independant_vars"],
        detailed=True,
    ).round(3)
...
def buildSingleCorrelogram(
    ...
    for treatment, group_df in subselection_df_ordered.groupby(
        ...
        methods = [
            getPearsonR,
            isSignificant(getPearsonPValue, p_value_threshold),
        ...
        columns = (
            columns if columns else CORRELOGRAM_COLUMN_ORDER[correlogram_type]
...
def plotCorrelograms(correlograms):
    ...
    )  # Put all the axes into a list at the same level
