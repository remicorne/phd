            ...
            for MA_denum in ratio_list_denum:
                ...
                if MA_num in exist_dict[BR].keys() and MA_denum in exist_dict[BR].keys():
                    ...
                    if len(overlap) > 0:
                        ...
                        df[ ratio_name , BR] [non_overlapping] = np.full( len(non_overlapping), np.nan)
                        
                        
                        ratio_match_ind_dict[BR][ ratio_name ] ['viable_indices'] = overlap
                        
                        group_spec_ind_dict, missing_groups = find_group_specific_indices(df, overlap, groups)
                        ...
                        print( MA_num, 'and', MA_denum, "don't have the same data")
...
def remove_groups_from_df (df_inc_ratios,  groups_to_drop = []):
    ...
    for i in groups_to_drop:
        ...
        df_dose_responce.drop(df_dose_responce.index[df_dose_responce['group', 'no'] == i], inplace=True)
        ...
        for BR, BR_dict in exist_dict.items():
                ...
                for comp, comp_dict in BR_dict.items():
                    ...
                    if treatment in comp_dict['missing_groups']:
                        break
                    x = np.array(df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][treatment]])
                    ...
                    if len(x) != len(x_):  #if outlier detected
                        print ('Grubbs Test exclusion for ', BR, ' ', comp, 'treatment group: ', treatment_dict[treatment])
                        print('mean of new set = ', x_.mean(), 'excluded point = ', np.setdiff1d(x,x_))

                        mouse_outliers = []
                        for outlier in np.setdiff1d(x,x_):
                            ind_of_outllier = df_inc_ratios.loc[df_inc_ratios[comp,BR] == float(outlier)].index[0]
                            mouse_outliers.append( df_inc_ratios.loc[ind_of_outllier]['mouse','no'] )
                        ...
                        df_inc_ratios[comp,BR].replace( np.setdiff1d(x,x_) , np.NaN , inplace=True)
...
def one_way_ANOVA_post_hoc_between_groups (df_inc_ratios, exist_dict, groups_to_drop = [], name = 'MA'):
    ...
    df_dose_responce =   remove_groups_from_df (df_inc_ratios,  groups_to_drop = groups_to_drop)                                                                 
    ...
    for BR, BR_dict in exist_dict.items():
       ...
       for comp, comp_dict in BR_dict.items():
           ...
           # if exist_dict[BR][comp]['missing_groups']) #dealing with missing groups
            if len(  exist_dict[BR][comp]['missing_groups'].intersection(set([1,2,3,4]))  ) > 0:###### need to debug and to add correct missing groups! SHIVA
               continue
           # if treatment in exist_dict[BR][comp]['group_specific_ind'].keys():
            ...
            F_value, p_value=scipy.stats.f_oneway(df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][1]] ,
                                             df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][2]] ,
                                             df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][3]] ,
            ...
            if p_value<0.05 :     #followed by post hoc
                ...
                mc = MultiComparison(df_dose_responce[comp, BR], df_dose_responce['group', 'no'])  ## could this be done through the dictionary
    ...
    df_one_way_anova.to_csv('/Users/jasminebutler/Desktop/dose_responce_anova_'+ name +'.csv')                
    writer.save()
    writer.close()                    
...
def student_t_test(exist_dict, df_inc_ratios, name = 'compounds'): 
    ...
    df_t_test_results = pd.DataFrame('NaN', index=[ 't_value', 'p_value'], columns=df_inc_ratios.columns[2:])  
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():     # to deal with groups that have n < 3
            ...
            if treatment in exist_dict[BR][comp]['group_specific_ind'].keys():
                ...
                x = df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][treatment]]
                ...
                if p < 0.05:
                    print(comp,'in', BR, 'is not normaly distributed for treatment ', treatment )
                    print ('p = ', p)
...
def save_shapiro_data_for_all_treatments (exist_dict, list_of_brain_regions, list_of_compounds, df_inc_ratios, list_of_groups):
    ...
    for treatment in list_of_groups:
            ...
            df_shapiro = calculate_shapiro_for_treatment (exist_dict, list_of_brain_regions, list_of_compounds, df_inc_ratios, treatment = treatment)
            treatment_str = str(treatment)
            df_shapiro.to_excel(writer, sheet_name =  'treatmenmt_group_'+treatment_str)
    writer.save()
    writer.close()  
    return
...
def two_way_ANOVA (exist_dict, df_inc_ratios, treatments_to_compare = [1,3,5,6], name = 'compounds'): 
    ...
    MDL_positive_index = df_factors.index[(df_factors['group', 'no'] == 5) | (df_factors['group', 'no'] == 6)]
    TCB2_positive_index_list = df_factors.index[(df_factors['group', 'no'] == 3) | (df_factors['group', 'no'] == 5)]
    
    
    bool_arr_TCB2 = np.full (len(df_factors) , False)
    bool_arr_TCB2[TCB2_positive_index_list] = True
    df_factors['AG', 'TCB2'] = bool_arr_TCB2 #add boolian column with TCB2 positive = true
    
    bool_arr_MDL = np.full (len(df_factors) , False)
    bool_arr_MDL[MDL_positive_index] = True
    df_factors['ANT', 'MDL'] =bool_arr_MDL #add boolian column with MDL positive = true
    ...
    writer_two_way = pd.ExcelWriter(name + 'two_way_ANOVA_agonist_antagonist.xlsx')     
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
            ...
            if len(  exist_dict[BR][comp]['missing_groups'].intersection(set(treatments_to_compare))  ) > 0:###### need to debug and to add correct missing groups! SHIVA
            ...
            for treatment in treatments_to_compare:
                indexes_to_keep.extend(list(comp_dict['group_specific_ind'][treatment]))
            
            d = { 'dv': df_factors[comp, BR][indexes_to_keep], 'ANT_MDL' : df_factors['ANT', 'MDL'][indexes_to_keep], 'AG_TCB2': df_factors['AG', 'TCB2'][indexes_to_keep], 'group': df_factors['group', 'no'][indexes_to_keep] }
            df_anova_working = pd.DataFrame(data=d)

            two_way_anova = pg.anova(data=df_anova_working, dv='dv', between=[('ANT_MDL'), ('AG_TCB2') ], detailed=True).round(3)
            
            p_list = two_way_anova['p-unc'][0:3]
            
            if min(p_list) < 0.05: 
                print(comp, ' ', BR , ' is significant for TWO-way anova', '   p=', min(p_list))
                ...
                F_value, p_value_one_way = scipy.stats.f_oneway(df_factors[comp, BR][comp_dict['group_specific_ind'][1]],
                                     df_factors[comp, BR][comp_dict['group_specific_ind'][3]],
                                     df_factors[comp, BR][comp_dict['group_specific_ind'][5]],
                                     df_factors[comp, BR][comp_dict['group_specific_ind'][6]])
                
                if p_value_one_way <0.05:
                    print(comp, ' ', BR, ' is significant for ONE-way anova', '   p=', p_value_one_way)
                    
                    # Nan_free_data = df_agonist_antagonist_colapsed_header.filter(items=['GABA_VM', 'group_no']).dropna()
                           
                    mc = MultiComparison(df_anova_working['dv'], df_anova_working['group'])
                    mc_results = mc.tukeyhsd()
                    print(mc_results)
                    df_Tukey = pd.DataFrame(data=mc_results._results_table.data[1:], columns=mc_results._results_table.data[0])
                    df_Tukey.to_excel(writer_tukey, sheet_name =  comp+'_'+BR)
                else:
                    print(comp, ' ', BR, ' is NOT significant for ONE-way-anova', '    p=', p_value_one_way)
            else:
                print(comp, ' ', BR, ' is NOT significant', '    p=', min(p_list))
...
def two_way_ANOVA_ALZ (exist_dict, df_inc_ratios, treatments_to_compare = [1,2,3,4], name = 'compounds'): 
    ...
    MDL_positive_index = df_factors.index[(df_factors['group', 'no'] == 2) | (df_factors['group', 'no'] == 4)]  ###AD positive (false for WT)
    TCB2_positive_index_list = df_factors.index[(df_factors['group', 'no'] == 3) | (df_factors['group', 'no'] == 4)]###old positive (false for young)
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
            ...
            if min(p_list) < 0.05: 
                ...
                F_value, p_value_one_way = scipy.stats.f_oneway(df_factors[comp, BR][comp_dict['group_specific_ind'][1]],
                                     df_factors[comp, BR][comp_dict['group_specific_ind'][2]],
                                     df_factors[comp, BR][comp_dict['group_specific_ind'][3]],
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
            ...
            if len(  exist_dict[BR][comp]['missing_groups'].intersection(set(treatments_to_compare))  ) > 0:
                print('missing treatment group ', exist_dict[BR][comp]['missing_groups'])
                continue
            ...
            for treatment in treatments_to_compare:
               ...
               mean_treatment.append(np.mean(df_compound_mean[df_compound_mean['group', 'no']==treatment][comp,BR]))
               SEM_treatment.append(np.mean(df_compound_SEM[df_compound_SEM['group', 'no']==treatment][comp,BR]))
            ...
            ax.set_title(comp.replace( '_' , '/' ) + ' in ' + BR , y=1.04, fontsize=34)
            # ax.yaxis.grid(True)
            if ratio == True:
            ...
            ax.set_title( comp.replace( '_' , '/' ) + ' in ' + BR , y=1.04, fontsize=34) #'+/- 68%CI'
            pdf.savefig(fig)
            plt.close(fig)
...
#                                    ratio = False, mouse_id = False):
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
            ...
            if len(  exist_dict[BR][comp]['missing_groups'].intersection(set(treatments_to_compare))  ) > 0: #if one of the treatment groups is missing skip entire plot for that compound and brain region
            ...
            #create relevant df
            d = { 'dv': df_inc_ratios[comp, BR][indexes_to_keep], 'group': df_inc_ratios['group', 'no'][indexes_to_keep], 'mouse' : df_inc_ratios['mouse', 'no'] }
            df_working = pd.DataFrame(data=d)
            df_working_labeled = df_working.replace({'group': treatment_dict}) #label groups
            ...
            if mouse_id == True:
                ...
                ax = sns.swarmplot(x="group", y="dv",  order = order , palette=color , data=df_working_labeled, hue = 'mouse', edgecolor='k', linewidth=1, linestyle='-')
                # https://seaborn.pydata.org/tutorial/color_palettes.html
                ax.legend_.remove()
                plt.legend(bbox_to_anchor=(0.99, 1), loc='upper left', borderaxespad= 0, ncol=2)

            else:
                    
                ax = sns.swarmplot(x="group", y="dv", palette=palette_labeled, order = order , data=df_working_labeled,  edgecolor='k', linewidth=1, linestyle='-')
...
def put_significnce_stars(ax, treatment_dict, test_path, data = None, 
    ...
    try:
        ...
        if len(df_significant.index) > 0:
            ...
            pairs = [(treatment_dict[i[1]["group1"]], treatment_dict[i[1]["group2"]]) for i in df_significant.iterrows()]
            

            annotator = Annotator( ax, pairs, data = data, 
                                  x = x, y = y, order= order)
            annotator.configure(text_format = "star", loc = "inside" , fontsize = 'xx-large') #https://stackoverflow.com/questions/64081570/matplotlib-marker-annotation-fontsize-not-shrinking-below-1pt-in-pdf
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
           ...
           for treatment in treatments_to_compare:
               ...
               indexes_to_keep.extend(list(comp_dict['group_specific_ind'][treatment]))  #collect all relevant indicies
           ...
           #create relevant df      
           d = { 'dv': df_inc_ratios[comp, BR][indexes_to_keep], 'group': df_inc_ratios['group', 'no'][indexes_to_keep] }
           df_working = pd.DataFrame(data=d)
           ...
           if df_t_test_results.loc['p_value', (comp,BR)] < p:
               ...
               test_results = add_stat_annotation(ax, data=df_working_labeled, x="group", y="dv",
                                                    box_pairs=box_pairs,
                                                    test='t-test_ind', text_format='star', # 'simple' reports p value or 'star' for stars only
    ...
    for r in df_corr_working.columns:
        for c in df_corr_working.columns:
            ...
            if Nan_r or Nan_c >0:
                ...
                print ('NaN count: ', Nan_r,' =  ', r,' and ', Nan_c,' = ',c)
                
                index_nan_r=df_corr_working[df_corr_working[r].isnull()].index.tolist()
                ...
                if Nan_clean_c > 0:     #if we still have Nans in the other column
                    index_nan_c=df_corr_working[df_corr_working[c].isnull()].index.tolist()
                    index_nan_c_no_duplicates = [x for x in index_nan_c if x not in index_nan_r] # only deleting indexes that have not been removed
...
#SIVA this df will have some NaN values and i dont really understand how it is being delt with with pandas .corr 
    ...
    for treatment in list_of_groups:        #create df for correlations of smetric data : each collumn is a factor to correlate maintining indicies
            ...
            for BR, BR_dict in exist_dict.items():
                ...
                index_of_group = df_inc_ratios.index[df_inc_ratios['group', 'no'] == treatment].tolist()        # get all indexes of treatment group
                ...
                if column_order == False:
                    ...
                    all_cols = [x for x in BR_dict.keys() if treatment not in list(exist_dict[BR][x]['missing_groups'])] # include only columns(compounds) without missing groups sufficient n in group/treatment
                    cols = [x for x in all_cols if len(exist_dict[BR][x]['group_specific_ind'][treatment]) > n_minimum] # transform cols to only include columns with n > n_minimum
                    
                    
                    df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=cols) #unordered but works for TCB2 and SNORD no column_order
                    ...
                    if 'HSER' in list(BR_dict.keys()):     # if you dont have a column order with all of your interest compounds
                    
                        df_corr_working = df_corr_working.drop(columns='HSER')   #remove HSER
                
                else: 
                    print ('column ordered:  ', column_order)
                    
                    all_cols = [x for x in BR_dict.keys() if treatment not in list(exist_dict[BR][x]['missing_groups'])] # include only columns without missing groups sufficient n in group/treatment
                    viable_cols = [x for x in all_cols if len(exist_dict[BR][x]['group_specific_ind'][treatment]) > n_minimum] # transform cols to only include columns with n > n_minimum
                    
                    cols = [x for x in column_order if x in viable_cols]     #list of BR in order 
                
                    df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=cols)      # create df to be filled for single BR and treatment
                ...
                for comp, comp_dict in BR_dict.items():
                    ...
                    if comp in list(df_corr_working.columns): # only fill columns tat have been created
                    
                        df_corr_working[comp] = df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][treatment]]     # fill df
                    else:
                        continue
                # count = count +1
                ...
                p_value_matrix = calculate_pvalues(df_corr_working)     #calculate p_values and set maske to P>0.05
                mask =  np.invert(np.tril(p_value_matrix <p_value))     
                
                heatmap = sns.heatmap(corr_matrix_BR, vmin=-1, vmax=1, annot=True, cmap='BrBG', mask=mask, annot_kws={"size":8})
                heatmap.set_title(BR + ' in ' + treatment_dict[treatment], fontdict={'fontsize':18}, pad=12);
                ax.set_xticklabels(
                    ax.get_xticklabels(),
                    rotation=45,
...
def pearson_correlations_within_compound (list_of_groups, exist_dict, df_inc_ratios, list_of_compounds, treatment_dict, name = 'compound', p_value = 0.05, n_minimum=8,
    ...
    for treatment in list_of_groups:        #create df for correlations of smetric data : each collumn is a factor to correlate maintining indicies
            ...
            for comp in list_of_compounds:
                ...
                existing_BR_for_comp = [x for x in exist_dict.keys() if comp in list(exist_dict[x].keys())]
                print( 'existing BR for comp = ', existing_BR_for_comp)
                
                all_cols = [x for x in existing_BR_for_comp if treatment not in list(exist_dict[x][comp]['missing_groups'])] # include only columns(BR's) without missing group/treatment
                cols = [x for x in all_cols if len(exist_dict[x][comp]['group_specific_ind'][treatment]) > n_minimum] # transform cols to only include columns with n > n_minimum
                ...
                if column_order == False:
                    ...
                    df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=cols) 
                    
                    if 'HSER' in list(exist_dict.keys()):     # if you dont have a column order with all of your interest compounds
                    ...
                    cols_ordered = [x for x in column_order if x in list(cols)]     #list of viable BR in order (compounds not in order will not be included )
                
                    df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=cols_ordered)      # create df to be filled for single BR and treatment
                ...
                # for BR, BR_dict in cols:
                    ...
                    if BR in list(df_corr_working.columns):
                        df_corr_working[BR] = df_inc_ratios[comp, BR][BR_dict[comp]['group_specific_ind'][treatment]] 
                ...
                p_value_matrix = calculate_pvalues(df_corr_working)     #calculate p_values and set maske to P>0.05
                p_value_matrix=p_value_matrix.dropna(axis=1,how='all')    # removing columns that have all 'NaN' vslues to match correlation matrix
                p_value_matrix=p_value_matrix.dropna(axis=0,how='all')    # removing rows that have all 'NaN' vslues
                
                mask =  np.invert(np.tril(p_value_matrix <p_value))     
                
                heatmap = sns.heatmap(corr_matrix_BR, vmin=-1, vmax=1, annot=True, cmap='BrBG', mask=mask, annot_kws={"size":8})
                heatmap.set_title(comp+'  in  '+ treatment_dict[treatment], fontdict={'fontsize':18}, pad=12);
                ax.set_xticklabels(
                    ax.get_xticklabels(),
                    rotation=45,
                    horizontalalignment='right');
...
def pearson_correlations_between_two_BR (treatment_dict, list_of_groups, exist_dict, 
                                           ...
                                           compounds_to_analise = ['DA','5HT','NA'],
                                           BR_to_analise = ['VL', 'VM']):
    ...
    for treatment in list_of_groups:        #create df for correlations of smetric data : each collumn is a factor to correlate maintining indicies
            ...
            df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=coulmns) 
                ...
                # if count == 1:
                    ...
                    for comp, comp_dict in BR_dict.items():
                        if comp in compounds_to_analise:   
                            ...
                            df_corr_working[comp+'_'+BR] = df_inc_ratios[comp, BR][comp_dict['group_specific_ind'][treatment]]     # fill df
                        else: 
                            continue
            ...
            corr_matrix_BR = corr_matrix_BR.drop(list(corr_matrix_BR.filter(regex='VL').columns), axis = 0) #searching column names as aways identical but drops rows with value
            corr_matrix_BR_plot = corr_matrix_BR.drop(list(corr_matrix_BR.filter(regex='VM').columns), axis = 1)#dropping columns with value
            
            p_value_matrix = p_value_matrix.drop(list(p_value_matrix.filter(regex='VL').columns), axis = 0) #searching column names as aways identical but drops rows with value
            p_value_matrix = p_value_matrix.drop(list(p_value_matrix.filter(regex='VM').columns), axis = 1)#dropping columns with value
            ...
            fig, ax = plt.subplots(figsize=(16, 10))
            heatmap = sns.heatmap(corr_matrix_BR_plot, vmin=-1, vmax=1, annot=True, cmap='BrBG', mask=mask, annot_kws={"size":16}) 
            # use matplotlib.colorbar.Colorbar object
            cbar = ax.collections[0].colorbar
            # here set the labelsize by 20
            cbar.ax.tick_params(labelsize=16)
            heatmap.set_title(BR_to_analise[0]+' and '+BR_to_analise[1]+'  '+str(treatment_dict[treatment]), fontdict={'fontsize':36}, pad=12, y = 1.03);
            ax.set_xticklabels(
                ax.get_xticklabels(),
                rotation=45,
...
def find_group_specific_indices(df, exist_ind, list_of_groups):
    ...
    exist_group =np.array( [gr_s[0] for gr_s in group_samples if gr_s[1] > 2])     #  groups exist when n < 3
    
    
    
    missing_groups = set(list_of_groups) - set(exist_group) #groups present i.e. list of groups *not groups
...
def calculate_mean_SD_SEM (df_inc_ratios, exist_dict):
    ...
    summary = { measure : pd.DataFrame ( groups , 
                                        columns = pd.MultiIndex.from_tuples([['group', 'no']])
                                        )
    ...
    for BR, BR_dict in exist_dict.items():
        ...
        for comp, comp_dict in BR_dict.items():
            ...
            for group, group_ind in comp_dict['group_specific_ind'].items():
                ...
                summary['mean'][comp, BR] [int(group) - 1] = df_inc_ratios[comp, BR][group_ind].mean() 
                summary['SD'][comp, BR] [int(group) - 1] =  df_inc_ratios[comp, BR][group_ind].std() 
...
#SIVA this df will have some NaN values and i dont really understand how it is being delt with with pandas .corr 
    ...
    for treatment in list_of_groups:        #create df for correlations of smetric data : each collumn is a factor to correlate maintining indicies
            ...
            for BR, BR_dict in exist_dict.items():
                ...
                df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=list(compounds_to_analise))      # create df to be filled for single BR and treatment
                ...
                heatmap = sns.heatmap(corr_matrix_BR, vmin=-1, vmax=1, annot=True, cmap='BrBG', mask=mask, annot_kws={"size":16})
                # use matplotlib.colorbar.Colorbar object
                cbar = ax.collections[0].colorbar
                # here set the labelsize by 20
                cbar.ax.tick_params(labelsize=16)
                heatmap.set_title(BR+' '+str(treatment_dict[treatment]), fontdict={'fontsize':36}, pad=12, y = 1.03);
                ax.set_xticklabels(
                    ax.get_xticklabels(),
                    rotation=45,
                    horizontalalignment='right', fontsize = 18);
    ...
    for treatment in list_of_groups:        #create df for correlations of smetric data : each collumn is a factor to correlate maintining indicies
            ...
            for comp in list_of_ratios:
                ...
                if column_order == False:
                    ...
                    df_corr_working = pd.DataFrame('NaN', index=index_of_group, columns=list(ratio_match_ind_dict.keys())) 
                    
                    if 'HSER' in list(ratio_match_ind_dict.keys()):     # if you dont have a column order with all of your interest compounds
                    
                        df_corr_working = df_corr_working.drop(columns='HSER')   #remove HSER
                
                else: 
                    print ('column ordered:  ', column_order)
                    cols = [x for x in column_order if x in list(ratio_match_ind_dict.keys())]     #list of BR in order 
                ...
                for BR, BR_dict in ratio_match_ind_dict.items():
                    ...
                    if str(comp) in compounds_in_this_BR:
                        df_corr_working[BR] = df_inc_ratios[comp, BR][BR_dict[comp]['group_specific_ind'][treatment]]     # fill df
                    else: 
                        print(BR , ' does not have ', comp)
