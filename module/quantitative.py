...
def processQuantitativeData(
    ...
    while not confirmed:
        ...
        if data[f"eliminated_{outlier_test}_outlier"].notna().all():
            ...
            stat_results = processQuantitativeStats(filename, data, p_value_threshold)

            ## DO ALL THE STATS AND PUT SIGNIFICANCE STARS
...
def processQuantitativeStats(
    ...
    for test in experiment_info["quantitative_statistics"]:
        is_significant, results = QUANTITATIVE_STAT_METHODS[test](
            data=data,
